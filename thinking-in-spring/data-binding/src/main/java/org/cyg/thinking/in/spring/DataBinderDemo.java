package org.cyg.thinking.in.spring;

import org.cyg.thinking.in.spring.ioc.overview.domain.Company;
...
public class DataBinderDemo {
    ...
    public static void main(String[] args) {
        ...
        // a. PropertyValues 存在 user 中 不存在属性值
        // DataBinder 特性一： 忽略未知的属性
        source.put("age", 18);
        // b. PropertyValue 中存在一个嵌套属性， 比如 company.name
        // DataBinder 特性二： 支持嵌套属性
        // Company company = new Company();
        // company.setName("gekkbing");
        // user.setCompany(company);
//        source.put("company", new Company());
        source.put("company.name", "geekbing");

        PropertyValues propertyValues = new MutablePropertyValues(source);

        // 1. 调整 IgnoreUnknownFields true (默认) -> false （抛出异常， age 字段不存在 User 类）
