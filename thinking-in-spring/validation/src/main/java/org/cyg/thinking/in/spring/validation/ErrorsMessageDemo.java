package org.cyg.thinking.in.spring.validation;
...
public class ErrorsMessageDemo {
    ...
    public static void main(String[] args) {
        ...
        // 3. 获取 Errors 中的 ObjectErrors 和 FieldError
        // FieldError is ObjectError
        List<ObjectError> globalErrors = errors.getGlobalErrors();
        List<FieldError> fieldErrors = errors.getFieldErrors();
        List<ObjectError> allErrors = errors.getAllErrors();
        // 4. 通过 ObjectErrors 和 FieldError 中的 code 和 args 来关联 MessageSource 实现
    ...
    static MessageSource createMessageSource() {
        ...
        messageSource.addMessage("user.properties.not.null", Locale.getDefault(),
                "User 所有属性不能为空");
        messageSource.addMessage("id.required", Locale.getDefault(),
                "the id of User must not be null.");
        messageSource.addMessage("name.required", Locale.getDefault(),
